...
public class MicrosoftPairHmmUnitTest {
    static final String pairHMMTestData = IntelGKLUtils.pathToTestResource("pairhmm-testdata.txt");

    @Test(enabled = true)
    public void simpleTest() {

        final boolean isloaded = new MicrosoftPairHmm().load(null);

        final MicrosoftPairHmm pairHmm = new MicrosoftPairHmm();
    ...
    public void dataFileTest() {
        // load native library
        final boolean isSupported = new MicrosoftPairHmm().load(null);
        Assert.assertTrue(isSupported);

        boolean[] udvals = {false, true};
        for(boolean useDbl : udvals) {
            // instantiate and initialize MicrosoftPairHmm
            final MicrosoftPairHmm pairHmm = new MicrosoftPairHmm();
